...
const resolveProjectRoot = (): string => {
  ...
  while (true) {
    ...
    if (parent === candidate) {
      throw new Error(
        `Unable to locate project root starting from ${__dirname}`,
      );
    }
    current = parent;
  }
...
export const RESOURCE_FALLBACK_PATH =
  process.env.RESOURCE_FALLBACK_PATH ??
  path.join(PROJECT_ROOT, 'server', 'resource.sample.json');
...
export const IS_PRODUCTION_RUNTIME =
  process.env.NODE_ENV === 'production' || RUNTIME_IN_DIST;
